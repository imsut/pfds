...
prop_head_tail ns = test' (foldr Q.cons Q.empty ns) ns
  where
...
prop_lookup [] = True
prop_lookup ns = test' (foldr Q.cons Q.empty ns) ns 0
  where
    test' :: Q.QuaternaryRAL Int -> [Int] -> Int -> Bool
    test' _ [] _ = True
    test' ral (m:ms) i = Q.lookup i ral == m && test' ral ms (i + 1)

prop_update :: [Int] -> Int -> Int -> Bool
prop_update [] _ _ = True
prop_update ns val r = test' (Q.update idx val $ foldr Q.cons Q.empty ns) ns 0
  where
    idx = r `mod` length ns
